...
import 'package:nucleo_casa_decor_android/features/specifier/data/models/acquisitions_item.dart';
import 'package:nucleo_casa_decor_android/features/specifier/data/models/user_details.dart';
...
class SpecifierController {
  ...
  final BehaviorSubject<List<AcquisitionsItem>> detailsController =
      BehaviorSubject.seeded([]);
  final BehaviorSubject<double> pointsController = BehaviorSubject.seeded(0.0);
  final BehaviorSubject<UserDetails?> userController = BehaviorSubject.seeded(
    null,
  );
  ...
  Future<void> getData() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        double valueTotal = item.fold(
          0.0,
          (sum, data) => sum + double.parse(data.valor!),
        );
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final match = jsonBody.firstWhere(
          (e) =>
              (e['cpf'] != null &&
                  e['cpf'].replaceAll(RegExp(r'\D'), '') == cpfOuCnpj) ||
              (e['cnpj'] != null &&
                  e['cnpj'].replaceAll(RegExp(r'\D'), '') == cpfOuCnpj),
          orElse: () => null,
        );
  ...
  Future<bool> updateUserData(UserDetails user, {XFile? imageFile}) async {
    ...
    var url = Uri.parse(
      "https://apicasadecor.com/api/especificador-editar/${user.id}/",
    );
    ...
    Map<String, dynamic> data = {
      if (user.segment != null && user.segment!.isNotEmpty)
        'seguimento': user.segment,
      if (user.phone != null && user.phone!.isNotEmpty) 'telefone': user.phone,
      if (user.cellPhone != null && user.cellPhone!.isNotEmpty)
