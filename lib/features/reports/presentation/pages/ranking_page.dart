...
class Purchase {
  ...
  String get formattedMultipliedValue => NumberFormat.currency(
    locale: 'pt_BR',
    symbol: 'R\$',
...
class ReleasesPage1 extends StatefulWidget {
  ...
  @override
  State<ReleasesPage1> createState() => _ReleasesPage1State();
}
...
class _ReleasesPage1State extends State<ReleasesPage1> {
  ...
  List<RankedData> get sortedRankedData =>
      _sumsByEspecificador.entries
          .map(
            (entry) => RankedData(entry.key, entry.value, entry.value / 1000),
          )
          .toList()
  ...
  Future<void> _fetchPurchases() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint('Failed to fetch purchases.');
      }
    } catch (e) {
  ...
  void _calculateSumsByEspecificador() {
    ...
    for (var purchase in _purchases) {
      ...
      if (_sumsByEspecificador.containsKey(especificador)) {
        _sumsByEspecificador[especificador] =
                  ...
                  Expanded(
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                          title: Text(
                            'Arquiteto(a): ${purchase.especificador}',
                          ),
                          subtitle: Column(
                            ...
                            children: [
                              ...
                              Text(
                                'Valor: ${purchase.formattedMultipliedValue}',
                              ),
                            ],
                          ),
                        );
