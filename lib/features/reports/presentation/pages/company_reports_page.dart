...
class _CompanyReportsScreenState extends State<CompanyReportsScreen>
  ...
  void _initializeCardAnimations() {
    for (var c in _cardAnimationControllers) {
      c.dispose();
    }
      ...
      for (var item in rawList) {
        try {
          ...
          final String nomeEmpresa =
              (empresa != null && empresa['nome'] != null)
                  ? empresa['nome']
  ...
  List<UserReport> get _filteredReports {
    var filtered =
        _userReports.where((report) {
          final nameMatch = report.favoriteStores
              .join(', ')
              .toLowerCase()
              .contains(_searchQuery.toLowerCase());
          final emailMatch = report.userEmail.toLowerCase().contains(
    ...
    if (_selectedMonth != 'Mês') {
      ...
      filtered =
          filtered
              .where((report) => report.createdAt.month == selectedIndex)
              .toList();
      ...
      for (var report in filtered) {
        ...
        if (!grouped.containsKey(empresa)) {
          ...
          grouped[empresa] = UserReport(
            ...
            favoriteStores: [empresa],
            lastActivity:
                existing.lastActivity.isAfter(report.lastActivity)
  ...
  Widget _buildStatsOverview() {
    ...
    final totalPoints = _userReports.fold<int>(
      0,
      (sum, report) => sum + report.totalPoints,
    );
    final totalSpent = _userReports.fold<double>(
      0,
      (sum, report) => sum + report.totalSpent,
    );
        ...
        children: [
          Row(
            children: [
              ElevatedButton.icon(
                onPressed:
                    () => PdfExportService.exportCompanyPdf(_filteredReports),
