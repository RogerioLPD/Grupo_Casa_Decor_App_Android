...
import 'package:nucleo_casa_decor_android/features/admin/data/models/client.dart';
...
class ClientNotifier extends StateNotifier<List<Client>> {
  ...
  Future<void> _saveClients() async {
    ...
    final clientsJson = jsonEncode(
      state.map((client) => client.toJson()).toList(),
    );
  ...
  Client? findByDocument(String document) {
    ...
    return state.cast<Client?>().firstWhere(
      (client) =>
          client!.cpfCnpj.replaceAll(RegExp(r'[^\d]'), '') == cleanDocument,
      orElse: () => null,
    );
  }
